...
Use an accordion when you want to keep a screen compact without hiding important context behind a separate route or modal. It works especially well for FAQs, settings groups, onboarding tips, and advanced form sections.

## Best Practices

- Keep each trigger label short and easy to scan
- Use it for secondary content, not your primary call to action
- Prefer a small number of sections so the interaction stays lightweight
- Open the first item by default if the content is important for first-time users
