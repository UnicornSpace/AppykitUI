...
const page = async (props: { params: Params }) => {
  ...
  if (courseSlug.length === 1) {
    ...
    const course = allCourses
      .filter(
        (item) =>
          item.file.dirname.toLowerCase() === courseSlug[0].toLowerCase()
      )
      .sort((a, b) => {
    ...
    const courseRegItem = coursesRegistry.find(
      ...
      <div className="xl:ml-20">
        ...
        <section className="mx-auto max-w-6xl  px-4 pb-10 md:pb-20 grid grid-flow-row grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
          {course.map((chapter, i) => (
            <Link
              key={i}
              href={`/course/${chapter.slugs[0]}/${re.test(chapter.slugs[1]) ? chapter.slugs[1].split(".")[1] : chapter.slugs[1]}`}
              className="group"
            >
              <Card
  ...
  if (courseSlug.length === 2) {
    ...
    // })
    console.log(
      allCourseChapters.map((c) => c.slugs[1]),
      "all course chapters"
    );
    ...
    const page = allCourseChapters.find((item) => {
      ...
      console.log();
      const convertedSlug = re.test(item.slugs[1])
        ? item.slugs[1].split(".")[1]
        : item.slugs[1];
